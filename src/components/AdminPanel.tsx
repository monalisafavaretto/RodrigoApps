...
import { 
  ...
  CheckCircle, Plus, Eye, History, Shield, Globe, Image as ImageIcon
} from 'lucide-react';
...
export default function AdminPanel({ onClose, adminEmail }: AdminPanelProps) {
  ...
  // New States for Pack de Imagens & PDF Courses
  ...
  // Form states under admin
  const [newImageName, setNewImageName] = useState('');
  const [newImageFile, setNewImageFile] = useState(''); // Base64
  const [newPdfName, setNewPdfName] = useState('');
  const [newPdfFile, setNewPdfFile] = useState(''); // Base64
  const [isUploadingImage, setIsUploadingImage] = useState(false);
  const [isUploadingPdf, setIsUploadingPdf] = useState(false);

  const fetchPacksAndCourses = () => {
    fetch('/api/pack-images')
      ...
      .catch(err => console.error("Error fetching pack images:", err));
    ...
    fetch('/api/pdf-courses')
      ...
      .catch(err => console.error("Error fetching pdf courses:", err));
  };
  ...
  const handlePackImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      compressImage(reader.result as string, 800)
        .then(compressed => {
  ...
  const handleAddPackImage = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newImageName.trim()) {
      alert("Por favor, digite o nome do Pack de Imagem.");
      return;
    }
    if (!newImageFile) {
    ...
    fetch('/api/admin/pack-images', {
      ...
      .then(data => {
        if (data.success) {
          showToast("Imagem adicionada ao Pack!");
          setNewImageName('');
          setNewImageFile('');
          
          // Reset file input cleanly
          const fileInput = document.getElementById('pack-image-input') as HTMLInputElement;
          if (fileInput) fileInput.value = '';

          fetchPacksAndCourses();
        } else {
          alert("Erro ao adicionar imagem ao Pack: " + (data.error || "Desconhecido"));
        }
      })
      .catch(err => {
        console.error(err);
        alert("Erro ao salvar imagem no Pack.");
      });
  };

  const handleDeletePackImage = (id: string, name: string) => {
    if (!confirm(`Deseja realmente excluir a imagem "${name}" do Pack?`)) return;

    fetch(`/api/admin/pack-images/${id}?admin_email=${encodeURIComponent(adminEmail)}`, {
      method: 'DELETE'
    })
      .then(safeJson)
      .then(data => {
        if (data.success) {
          ...
          alert("Erro ao excluir imagem: " + (data.error || "Desconhecido"));
        }
      })
      .catch(err => {
        console.error(err);
        alert("Erro de rede ao excluir imagem.");
      });
  };
  ...
  const handlePdfSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      setNewPdfFile(reader.result as string);
      setIsUploadingPdf(false);
      showToast("PDF processado com sucesso!");
    };
    reader.onerror = () => {
      setIsUploadingPdf(false);
  ...
  const handleAddPdfCourse = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPdfName.trim()) {
      alert("Por favor, digite o nome do Curso / Apostila.");
      return;
    }
    if (!newPdfFile) {
      alert("Por favor, selecione uma apostila em PDF.");
      return;
    }

    fetch('/api/admin/pdf-courses', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        admin_email: adminEmail,
      ...
      .then(data => {
        if (data.success) {
          showToast("Curso/Apostila adicionada com sucesso!");
          setNewPdfName('');
          setNewPdfFile('');

          // Reset file input cleanly
          const fileInput = document.getElementById('pdf-course-input') as HTMLInputElement;
          if (fileInput) fileInput.value = '';

          fetchPacksAndCourses();
        } else {
          alert("Erro ao salvar apostila: " + (data.error || "Desconhecido"));
        }
      })
      .catch(err => {
        console.error(err);
        alert("Erro ao conectar ao servidor para salvar PDF.");
      });
  };

  const handleDeletePdfCourse = (id: string, name: string) => {
    if (!confirm(`Deseja realmente excluir a apostila "${name}"?`)) return;

    fetch(`/api/admin/pdf-courses/${id}?admin_email=${encodeURIComponent(adminEmail)}`, {
      method: 'DELETE'
    })
      .then(safeJson)
      .then(data => {
        if (data.success) {
          showToast("Curso/Apostila removido com sucesso.");
          fetchPacksAndCourses();
        } else {
          alert("Erro ao excluir PDF: " + (data.error || "Desconhecido"));
        }
      })
      .catch(err => {
        console.error(err);
        alert("Erro ao conectar ao servidor para excluir PDF.");
      });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0b0c10] text-zinc-100 overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {/* Gestão de Assets: Pack de Imagens e Cursos/Apostilas */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-6">
          
          {/* Pack de Imagens Tool */}
          <section className="bg-[#12131a] border border-zinc-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-[#a5b4fc] flex items-center gap-2 mb-2">
              <ImageIcon className="w-4.5 h-4.5 text-indigo-400" />
              Upload & Gestão do Pack de Imagens
            </h3>
            <p className="text-xs text-zinc-400 pb-1">
              Imagens adicionadas aqui aparecerão automaticamente para os membros finais em alta definição na aba "Pack de imagens" do menu lateral.
            </p>

            <form onSubmit={handleAddPackImage} className="space-y-3 bg-[#0a0b0d] p-4 rounded-lg border border-zinc-905">
              <div>
                <label className="block text-[10px] text-zinc-500 uppercase font-semibold font-mono mb-1">Nome da Imagem</label>
                <input
                  type="text"
                  placeholder="Ex: Molde de Urso - Realista"
                  className="w-full bg-[#12131a] border border-zinc-800 rounded-lg p-2 text-xs outline-none text-zinc-200"
                  value={newImageName}
              ...
              <div>
                <label className="block text-[10px] text-zinc-500 uppercase font-semibold font-mono mb-1">Escolher Imagem (JPG, PNG)</label>
                <input
                  id="pack-image-input"
                  type="file"
                  accept="image/*"
                  className="w-full text-xs text-zinc-400 file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-600/20 file:text-indigo-400 hover:file:bg-indigo-600/30 cursor-pointer"
                  onChange={handlePackImageSelect}
                />
              </div>

              {isUploadingImage && (
                <div className="text-[11px] text-yellow-500 animate-pulse font-medium">Processando e compactando imagem...</div>
              )}

              {newImageFile && (
                <div className="mt-2.5">
                  <p className="text-[10px] text-zinc-500 mb-1 font-mono">Pré-visualização:</p>
                  <img src={newImageFile} className="h-16 w-16 object-cover rounded-lg border border-zinc-800" alt="Preview" referrerPolicy="no-referrer" />
                </div>
              )}

              <button
                type="submit"
                disabled={isUploadingImage || !newImageFile || !newImageName.trim()}
                className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white font-bold rounded-lg text-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5 shadow duration-150"
              >
                <Plus className="w-4 h-4" />
                Adicionar Imagem ao Pack
              </button>
            </form>

            <h4 className="text-[10px] text-[#a5b4fc] uppercase tracking-widest font-bold pt-1">Imagens Atuais ({packImages.length})</h4>
            <div className="grid grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-1">
              {packImages.length === 0 ? (
                <div className="col-span-2 text-center py-10 text-xs text-zinc-600 bg-[#0a0b0d] rounded-lg">Nenhuma imagem cadastrada no pack de de imagens.</div>
              ) : (
                packImages.map((img) => (
                  <div key={img.id} className="bg-[#0a0b0d] border border-zinc-900 rounded-lg p-2 flex flex-col gap-2 relative group hover:border-zinc-800 transition-all">
                    <img src={img.url} className="w-full h-24 object-cover rounded-md bg-[#12131a]" alt={img.name} referrerPolicy="no-referrer" />
                    <div className="min-w-0">
                      <p className="text-[11px] font-bold text-zinc-350 truncate" title={img.name}>{img.name}</p>
                      <p className="text-[9px] text-zinc-500 font-mono mt-0.5">{new Date(img.createdAt).toLocaleDateString('pt-BR')}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleDeletePackImage(img.id, img.name)}
                      className="absolute top-3 right-3 bg-red-600 hover:bg-red-500 text-white p-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer shadow-md"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
          ...
          {/* Cursos / Apostilas PDF Tool */}
          <section className="bg-[#12131a] border border-zinc-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-[#a5b4fc] flex items-center gap-2 mb-2">
              <FileText className="w-4.5 h-4.5 text-indigo-400" />
              Upload & Gestão de Apostilas/Cursos PDF
            </h3>
            <p className="text-xs text-zinc-400 pb-1">
              Cursos e Apostilas adicionadas aqui ficam disponíveis para download imediato para os usuários na aba "Apostilas/Cursos".
            </p>

            <form onSubmit={handleAddPdfCourse} className="space-y-3 bg-[#0a0b0d] p-4 rounded-lg border border-zinc-905">
              <div>
                <label className="block text-[10px] text-zinc-500 uppercase font-semibold font-mono mb-1">Nome do Curso / Apostila</label>
                <input
                  type="text"
                  placeholder="Ex: Treinamento Completo de Resina Epóxi"
                  className="w-full bg-[#12131a] border border-zinc-800 rounded-lg p-2 text-xs outline-none text-zinc-200"
                  value={newPdfName}
              ...
              <div>
                <label className="block text-[10px] text-zinc-500 uppercase font-semibold font-mono mb-1">Escolher PDF (Abaixo de 50MB)</label>
                <input
                  id="pdf-course-input"
                  type="file"
              ...
              {isUploadingPdf && (
                <div className="text-[11px] text-yellow-500 animate-pulse font-medium font-mono">Lendo e codificando arquivo PDF...</div>
              )}

              {newPdfFile && (
                <div className="text-[10px] text-emerald-450 font-semibold flex items-center gap-1.5 font-mono">
              ...
              <button
                type="submit"
                disabled={isUploadingPdf || !newPdfFile || !newPdfName.trim()}
                className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white font-bold rounded-lg text-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5 shadow duration-150"
              >
                <Plus className="w-4 h-4" />
                Adicionar Apostila / Curso PDF
              </button>
            </form>

            <h4 className="text-[10px] text-[#a5b4fc] uppercase tracking-widest font-bold pt-1">Apostilas Ativas ({pdfCourses.length})</h4>
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {pdfCourses.length === 0 ? (
                <div className="text-center py-10 text-xs text-zinc-650 bg-[#0a0b0d] rounded-lg">Nenhuma apostila disponível.</div>
              ) : (
                pdfCourses.map((pdf) => (
                  <div key={pdf.id} className="bg-[#0a0b0d] border border-zinc-900 rounded-lg p-3 flex items-center justify-between relative group hover:border-zinc-800 transition-all">
                    <div className="flex items-center gap-2.5 min-w-0 pr-4">
                      <div className="w-9 h-9 bg-indigo-600/10 border border-indigo-500/20 flex items-center justify-center rounded-lg text-indigo-400 shrink-0">
                        <FileText className="w-5 h-5" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-[11px] font-bold text-zinc-300 truncate" title={pdf.name}>{pdf.name}</p>
                        <p className="text-[9px] text-zinc-500 font-mono mt-0.5">{new Date(pdf.createdAt).toLocaleDateString('pt-BR')}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleDeletePdfCourse(pdf.id, pdf.name)}
                      className="bg-red-650 hover:bg-red-500 text-white p-1.5 rounded-md cursor-pointer transition-colors shadow-sm shrink-0"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
